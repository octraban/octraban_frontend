      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4 }}>
            {isReentrancy && (
              <span>
                A reentrancy guard prevented recursive re-entry into a locked contract state.
              </span>
            )}
          ...
          {result.trapEventIndex != null && (
            <div style={{ fontSize: 12, marginTop: 6 }}>
              ...
              {result.trapMessage && (
                <span style={{ color: "var(--muted)" }}>: <em>{result.trapMessage}</em></span>
              )}
            </div>
          )}
        </div>
            ...
            <tbody>
              {result.findings.map((f, i) => (
                <tr key={i} style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <td style={{ ...td, fontFamily: "monospace", color: borderColor }}>{f.error}</td>
